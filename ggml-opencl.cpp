...
static cl_mem ggml_cl_pool_malloc(size_t size, size_t * actual_size) {
    ...
    cl_mem mem;
    CL_CHECK((mem = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, size, NULL, &err), err));
    *actual_size = size;
    return mem;
}
            ...
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    ...
                    clblast::StatusCode status = clblast::Gemm<cl_float>(
                        clblast::Layout::kColMajor, clblast::Transpose::kYes, clblast::Transpose::kNo, ne01, ne11, ne10,
                        alpha, d_X, x_offset, ne00, d_Y, 0, ne10, beta, d_D, 0, ne01, &queue, &ev_sgemm);
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    ...
                    // copy src1 to device
                    CL_CHECK(
                        clEnqueueWriteBuffer(queue, d_Y, false, 0, sizeof(ggml_fp16_t) * y_ne, tmp, 0, NULL, NULL));
                    ...
                    CL_CHECK(
                        clEnqueueReadBuffer(queue, d_D, true, 0, sizeof(ggml_fp16_t) * d_ne, tmp, 1, &ev_sgemm, NULL));
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                if (!mul_mat_vec) {
                    ...
                    CL_CHECK(clEnqueueNDRangeKernel(queue, *to_fp32_cl, 1, &offset, &global, local > 0 ? &local : NULL,
                                                    events.size(), !events.empty() ? events.data() : NULL, NULL));
                }
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
                        ...
                        CL_CHECK(clEnqueueNDRangeKernel(queue, *dmmv, 1, &offset, &global, &local, events.size() - 1,
                        ...
                        clblast::StatusCode status =
                            clblast::Gemm<cl_float>(clblast::Layout::kColMajor, clblast::Transpose::kYes,
                                                    clblast::Transpose::kNo, ne01, ne11, ne10, alpha, d_X, 0, ne00, d_Y,
                                                    0, ne10, beta, d_D, 0, ne01, &queue, events.data() + ev_idx++);
                    ...
                    cl_int err;
                    clFinish(queue);
                    char *d = (char *)dst->data + i12 * dst->nb[2] + i13 * dst->nb[3];
                    float *tmp = (float *)clEnqueueMapBuffer(queue, d_D, CL_TRUE, CL_MAP_READ, 0, d_size, 1,
                                                             events.data() + ev_idx - 1, NULL, &err);
                    CL_CHECK(err);
                    for (int ii = 0; ii < dst->ne[1]; ++ii) {
                        memcpy(d + ii * dst->nb[1], tmp + ii * ne01, sizeof(float) * dst->ne[0]);
                    }
                    CL_CHECK(clEnqueueUnmapMemObject(queue, d_D, tmp, 0, NULL, NULL));
                    for (auto *event : events) {
                        clReleaseEvent(event);
                    }
...
static bool ggml_cl_mul_mat_use_f16(const struct ggml_tensor *src0, const struct ggml_tensor *src1,
...
size_t ggml_cl_mul_mat_get_wsize(const struct ggml_tensor *src0, const struct ggml_tensor *src1,
                                 struct ggml_tensor *dst) {
...
// combo
static void combo_cl_mul_mat_q_f32(const ggml_tensor *src0, const ggml_tensor *src1, ggml_tensor *dst) {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    const int64_t ne02 = src0->ne[2];
    const int64_t ne03 = src0->ne[3];
    ...
    const size_t x_ne = ne01 * ne00;
    const int y_ne = ne11 * ne10;
    const int d_ne = ne11 * ne01;

    const bool src0_cont = ggml_is_contiguous(src0);
    const bool dx_need_alloc = !src0_cont;

    const bool src1_cont = ggml_is_contiguous(src1);
    // GGML_ASSERT(src1_cont);
    const bool dy_need_alloc = !src1_cont;

    const bool dst_cont_strict = ggml_is_contiguous(dst);
    const bool dd_need_alloc = !dst_cont_strict;

    cl_int err;
    cl_mem d_X, d_Y, d_D, d_A;
    size_t x_size, a_size, y_size, d_size;
    if (!mul_mat_vec) {
        d_A = ggml_cl_pool_malloc(sizeof(float) * x_ne, &a_size);
    }

    if (dx_need_alloc) {
        const int x_bps = x_ne / ggml_blck_size(type); // blocks per 2D slice
        const size_t q_sz = ggml_type_size(type) * x_bps;
        d_X = ggml_cl_pool_malloc(q_sz, &x_size);
    }

    size_t dy_ld = src1->nb[1] / ggml_type_size(src1->type);
    if (dy_need_alloc) {
        d_Y = ggml_cl_pool_malloc(sizeof(float) * y_ne, &y_size);
        dy_ld = ne10;
    }

    size_t dst_ld = dst->nb[1] / ggml_type_size(dst->type);
    if (dd_need_alloc) {
        d_D = ggml_cl_pool_malloc(sizeof(float) * d_ne, &d_size);
        dst_ld = ne01;
    }
    ...
    size_t ev_idx = 0;
    std::vector<cl_event> events;
    events.reserve(CL_MAX_EVENT_SIZE * 5);
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                if (dx_need_alloc) {
                    // copy src0 to device
                    events.emplace_back();
                    CL_CHECK(ggml_cl_h2d_tensor_2d(queue, d_X, 0, src0, i03, i02, events.data() + ev_idx++));
                } else {
                    char *src0_host = (char *)src0->data + i03 * src0->nb[3] + i02 * src0->nb[2];
                    CL_CHECK((d_X = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                                                   src0->ne[1] * src0->nb[1], src0_host, &err),
                              err));
                }
                ...
                if (!mul_mat_vec) {
                    ...
                    if (dx_need_alloc) {
                        wait_num = 1;
                        wait = events.data() + ev_idx - 1;
                    }
                    events.emplace_back();
                    CL_CHECK(clSetKernelArg(*to_fp32_cl, 0, sizeof(cl_mem), &d_X));
                    CL_CHECK(clSetKernelArg(*to_fp32_cl, 1, sizeof(cl_mem), &d_A));
                    CL_CHECK(clEnqueueNDRangeKernel(queue, *to_fp32_cl, 1, &offset, &global, local > 0 ? &local : NULL,
                                                    wait_num, wait, events.data() + ev_idx++));
                }
                events.emplace_back();
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    if (dy_need_alloc) {
                        ...
                        char *src1_host = (char *)src1->data + i13 * src1->nb[3] + i12 * src1->nb[2];
                        CL_CHECK((d_Y = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                                                       src1->nb[1] * src1->ne[1], src1_host, &err),
                                  err));
                    }

                    if (!dd_need_alloc) {
                        char *dst_host = (char *)dst->data + i13 * dst->nb[3] + i12 * dst->nb[2];
                        d_size = dst->ne[1] * dst->nb[1];
                        CL_CHECK((d_D = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, d_size,
                                                       dst_host, &err),
                                  err));
                    }
                    CL_CHECK(clFinish(queue));
                    if (mul_mat_vec) {
                        ...
                        CL_CHECK(clSetKernelArg(*dmmv, 0, sizeof(cl_mem), &d_X));
                        CL_CHECK(clSetKernelArg(*dmmv, 1, sizeof(float) * local, NULL));
                        CL_CHECK(clSetKernelArg(*dmmv, 2, sizeof(cl_mem), &d_Y));
                        CL_CHECK(clSetKernelArg(*dmmv, 3, sizeof(cl_mem), &d_D));
                        CL_CHECK(clSetKernelArg(*dmmv, 4, sizeof(cl_int), &ncols));
                        CL_CHECK(clEnqueueNDRangeKernel(queue, *dmmv, 1, &offset, &global, &local, 0, NULL,
                                                        events.data() + ev_idx++));
                    } else {
                        clblast::StatusCode status =
                            clblast::Gemm<cl_float>(clblast::Layout::kColMajor, clblast::Transpose::kYes,
                                                    clblast::Transpose::kNo, ne01, ne11, ne10, alpha, d_A, 0, ne00, d_Y,
                                                    0, dy_ld, beta, d_D, 0, dst_ld, &queue, events.data() + ev_idx++);
                        if (status != clblast::StatusCode::kSuccess) {
                            GGML_ASSERT(false);
                        }
                    }
    ...
    if (dd_need_alloc) {
        ...
        cl_mem d_dst;
        CL_CHECK(
            (d_dst = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, dst_size, dst->data, &err), err));
        CL_CHECK((clEnqueueMapBuffer(queue, d_dst, CL_TRUE, CL_MAP_READ, 0, dst_size, 0, 0, NULL, &err), err));
    }
}

static void combo_cl_mul_mat_f16(const ggml_tensor *src0, const ggml_tensor *src1, ggml_tensor *dst) {
    GGML_ASSERT(fp16_support);
    ...
    if (!src0_cont) {
        ...
        x_ld = src0->nb[1] / ggml_type_size(src0->type) * ggml_blck_size(src0->type);
    }
    cl_mem d_Y = ggml_cl_pool_malloc(sizeof(ggml_fp16_t) * y_ne, &y_size);
    cl_mem d_D = ggml_cl_pool_malloc(sizeof(ggml_fp16_t) * d_ne, &d_size);
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    // convert src1 to fp16
                    // TODO: use multiple threads
                    tmp = (ggml_fp16_t *)clEnqueueMapBuffer(queue, d_Y, CL_TRUE, CL_MAP_WRITE, 0, y_size, 0, nullptr,
                                                            nullptr, &err);
                    CL_CHECK(err);
                    char *src1i = (char *)src1->data + i13 * nb13 + i12 * nb12;
                    if (src1_cont_rows) {
                    ...
                    CL_CHECK(clEnqueueUnmapMemObject(queue, d_Y, tmp, 0, NULL, NULL));
                    ...
                    clblast::StatusCode status = clblast::Gemm<cl_half>(
                        clblast::Layout::kColMajor, clblast::Transpose::kYes, clblast::Transpose::kNo, ne01, ne11, ne10,
                        alpha, d_X, 0, x_ld, d_Y, 0, ne10, beta, d_D, 0, ne01, &queue, &ev_sgemm);
                    ...
                    tmp = (ggml_fp16_t *)clEnqueueMapBuffer(queue, d_D, CL_TRUE, CL_MAP_READ, 0, d_size, 1, &ev_sgemm,
                                                            NULL, &err);
                    CL_CHECK(err);
                    for (int ii = 0; ii < dst->ne[1]; ++ii) {
                        float *target = (float *)(d + ii * dst->nb[1]);
                        ggml_fp16_to_fp32_row(tmp + ii * ne01, target, dst->ne[0]);
                    }
                    CL_CHECK(clEnqueueUnmapMemObject(queue, d_D, tmp, 0, NULL, NULL));
                }
            }
...
static void combo_cl_mul_mat_f32(const ggml_tensor *src0, const ggml_tensor *src1, ggml_tensor *dst) {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    const int64_t ne02 = src0->ne[2];
    const int64_t ne03 = src0->ne[3];
    ...
    size_t d_size;
    cl_mem d_X, d_Y, d_D;
    size_t x_ld = ne00, y_ld = ne10, d_ld = d_ld = dst->nb[1] / ggml_type_size(dst->type) * ggml_blck_size(dst->type);;
    if (!src0_cont) {
        d_X = ggml_cl_pool_malloc(sizeof(float) * x_ne, &x_size);
    } else {
        x_ld = src0->nb[1] / ggml_type_size(src0->type) * ggml_blck_size(src0->type);
    ...
    if (!src1_cont) {
        ...
        y_ld = src1->nb[1] / ggml_type_size(src1->type) * ggml_blck_size(src1->type);
    }
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
            ...
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    ...
                    if (dst_cont) {
                        d_size = dst->ne[1] * dst->nb[1];
                        char *dst_host = (char *)dst->data + i13 * dst->nb[3] + i12 * dst->nb[2];
                        CL_CHECK((d_D = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, d_size,
                                                       dst_host, &err),
                                  err));
                    }
                    ...
                    clblast::StatusCode status = clblast::Gemm<cl_float>(
                        clblast::Layout::kColMajor, clblast::Transpose::kYes, clblast::Transpose::kNo, ne01, ne11, ne10,
                        alpha, d_X, 0, x_ld, d_Y, 0, y_ld, beta, d_D, 0, d_ld, &queue, &ev_sgemm);
                    ...
                    if (!dst_cont) {
                        CL_CHECK(clFinish(queue));
                        char *d = (char *)dst->data + i12 * dst->nb[2] + i13 * dst->nb[3];
                        float *tmp = (float *)clEnqueueMapBuffer(queue, d_D, CL_TRUE, CL_MAP_READ, 0, d_size, 0, NULL,
                                                                 NULL, &err);
                        CL_CHECK(err);
                        for (int ii = 0; ii < dst->ne[1]; ++ii) {
                            memcpy(d + ii * dst->nb[1], tmp + ii * ne01, sizeof(float) * dst->ne[0]);
        ...
        if (!dst_cont) {
            ...
            cl_mem d_dst;
            CL_CHECK(
                (d_dst = clCreateBuffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, dst_size, dst->data, &err),
                 err));
            CL_CHECK((clEnqueueMapBuffer(queue, d_dst, CL_TRUE, CL_MAP_READ, 0, dst_size, 0, 0, NULL, &err), err));
        }
    }
...
static bool combo_cl_mul_mat_use_f16(const struct ggml_tensor *src0, const struct ggml_tensor *src1,
                                     struct ggml_tensor *dst) {
    // If device doesn't support FP16
    if (!fp16_support) {
        return false;
    ...
    size_t src0_sz = ggml_nbytes(src0);
    bool src0_cont = ggml_is_contiguous(src0);
    size_t src1_sz = ggml_nbytes(src1);
    bool src1_cont = ggml_is_contiguous(src1);

    // mul_mat_q: src0 is converted to fp32 on device
    size_t mul_mat_q_transfer = (src0_cont ? 0 : src0_sz) + sizeof(float) * ggml_nelements(src0);

    // mul_mat_f16: src1 is converted to fp16 on cpu
    size_t mul_mat_f16_transfer = (src1_cont ? 0 : src1_sz) + sizeof(ggml_fp16_t) * ggml_nelements(src1);
...
void combo_cl_mul_mat(const struct ggml_tensor * src0, const struct ggml_tensor * src1, struct ggml_tensor * dst) {
    GGML_ASSERT(combo_cl_can_mul_mat(src0, src1, dst));

    if (src0->type == GGML_TYPE_F32) {
        combo_cl_mul_mat_f32(src0, src1, dst);
    }
    else if (src0->type == GGML_TYPE_F16) {
        if (ggml_cl_mul_mat_use_f16(src0, src1, dst)) {
            combo_cl_mul_mat_f16(src0, src1, dst);
        } else {
            combo_cl_mul_mat_q_f32(src0, src1, dst);
        }
    } else if (ggml_is_quantized(src0->type)) {
        ggml_cl_mul_mat_q_f32(src0, src1, dst);
    } else {
