...
static void combo_compute_forward_mul_mat_opencl(const struct ggml_compute_params *params,
    ...
    if (params->ith == 0 && params->type == GGML_TASK_COMPUTE) {
        combo_cl_mul_mat(src0, src1, dst);
    }
#else
    GGML_ASSERT(false);
#endif
...
static void combo_compute_forward_mul_mat_cpu(const struct ggml_compute_params *params, const struct ggml_tensor *src0,
...
#if defined(GGML_USE_CLBLAST)
    if (combo_cl_can_mul_mat(src0, src1, dst)) {
      if (params->type == GGML_TASK_INIT) {
        return;
      }
      if (params->ith == 0 && params->type == GGML_TASK_COMPUTE) {
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case COMBO_OP_MUL_MAT_CPU:
            {
                combo_compute_forward_mul_mat_cpu(params, tensor->src[0], tensor->src[1], tensor);
            } break;
        case COMBO_OP_MUL_MAT_OPENCL:
            {
                combo_compute_forward_mul_mat_opencl(params, tensor->src[0], tensor->src[1], tensor);
            } break;
        case GGML_OP_COUNT:
            {
                GGML_ASSERT(false);
...
#if defined(GGML_USE_CLBLAST)
                if (combo_cl_can_mul_mat(node->src[0], node->src[1], node)) {
...
// #if defined(GGML_USE_CLBLAST)
//                     if (combo_cl_can_mul_mat(node->src[0], node->src[1],
//                                              node)) {
//                       cur = 0;
//                     } else
// #endif
...
// combo
static struct combo_job_queue *create_combo_job_queue(void) {
    struct combo_job_queue *job_que = (struct combo_job_queue *)malloc(sizeof(struct combo_job_queue));
    job_que->head = job_que->tail = NULL;
    job_que->size = 0;
    GGML_ASSERT(pthread_spin_init(&job_que->spx, 0) == 0);
    return job_que;
}

static int delete_combo_job_queue(struct combo_job_queue *job_que) {
    pthread_spin_lock(&job_que->spx);
    while (job_que->head != NULL) {
        struct combo_job *next = job_que->head->next;
        free(job_que->head);
        job_que->head = next;
    }
    pthread_spin_unlock(&job_que->spx);
    pthread_spin_destroy(&job_que->spx);
    free(job_que);
    return 0;
}

static inline int add_job(struct combo_job_queue *job_que, struct ggml_tensor *tensor,
    struct combo_tensor_data *data, struct combo_decode_history *history) {
    struct combo_job *tmp = malloc(sizeof(struct combo_job));
    // GGML_ASSERT(tmp);
    tmp->job = tensor;
    tmp->data = data;
    tmp->history = history;
    ...
    pthread_spin_lock(&job_que->spx);
    job_que->size++;
    if (job_que->tail == NULL) {
        job_que->head = job_que->tail = tmp;
    } else {
        job_que->tail->next = tmp;
        job_que->tail = tmp;
    }
    pthread_spin_unlock(&job_que->spx);
    return 0;
...
static inline int get_job(struct combo_job_queue *job_que, struct ggml_tensor **work_node, struct combo_tensor_data **data, struct combo_decode_history **history) {
    pthread_spin_lock(&job_que->spx);
    if (job_que->head != NULL) {
        job_que->size--;
        *work_node = job_que->head->job;
        *data = job_que->head->data;
        *history = job_que->head->history;
        struct combo_job *tmp = job_que->head;
        job_que->head = job_que->head->next;
        free(tmp);
    }
    if (job_que->head == NULL) {
        job_que->tail = NULL;
...
static inline bool empty(struct combo_job_queue *job_que) {
    bool empty = false;
    pthread_spin_lock(&job_que->spx);
    empty = job_que->head == NULL ? true : false;
    pthread_spin_unlock(&job_que->spx);
    return empty;
}

struct combo_compute_state_shared {
    struct ggml_tensor *work_node;
    struct combo_tensor_data *data;
    struct combo_decode_history *history;
    int nth;
};
struct combo_compute_state {
    int ith;
    uint32_t numa_cores;
    struct combo_thread_pool_handler *handler;
    struct combo_compute_state_shared *shared;
};

static ggml_thread_t combo_compute_thread(void *data) {
    struct combo_compute_state *shared_data = (struct combo_compute_state *)data;
    pthread_cond_t *cv = &shared_data->handler->cv;
    pthread_cond_t *cv_main = &shared_data->handler->cv_main;
    pthread_mutex_t *lock = &shared_data->handler->lock;
    int n_threads = shared_data->handler->n_threads;
    if (n_threads == 1) {
        set_numa_thread_affinity(3, shared_data->numa_cores);
    } else {
        set_numa_thread_affinity(shared_data->ith, n_threads);
    }
    struct ggml_compute_params params = {
        /*.ith   =*/0,
        /*.type  =*/GGML_TASK_COMPUTE,
        /*.nth   =*/0,
        /*.wsize =*/shared_data->handler->wsize,
        /*.wdata =*/shared_data->handler->wbuff,
    };

    while (true) {
        pthread_mutex_lock(lock);
        if (shared_data->handler->n_wait == n_threads - 1) {
            pthread_mutex_unlock(lock);
            while (true) {
                // 最后完成的进程收尾
                if (shared_data->shared->work_node != NULL) {
                    if (GGML_OP_HAS_FINALIZE[shared_data->shared->work_node->op]) {
                        params.type = GGML_TASK_FINALIZE;
                        params.ith = 0;
                        params.nth = 1;
                        params.wsize = shared_data->handler->wsize;
                        params.wdata = shared_data->handler->wbuff;
                        ggml_compute_forward(&params, shared_data->shared->work_node);
                    }
                    // 记录耗时
                    if (shared_data->shared->work_node->time_us != NULL) {
                        *(shared_data->shared->work_node->time_us) =
                            ggml_time_us() - *(shared_data->shared->work_node->time_us);
                    }
                    // node的计算全部完成后，保存cpu/igpu比例不同的最优和次优记录
                    struct combo_tensor_data *node_data = shared_data->shared->data;
                    if (__atomic_load_n(shared_data->shared->work_node->n_wait, __ATOMIC_SEQ_CST) == 1 &&
                        node_data != NULL && shared_data->shared->history != NULL) {
                        node_data->total_time = MAX(node_data->cpu_time, node_data->igpu_time);
                        pthread_spin_lock(&shared_data->shared->history->spx);
                        struct combo_tensor_data *bst = &shared_data->shared->history->bst;
                        struct combo_tensor_data *cur = &shared_data->shared->history->current;
                        if (bst->total_time == 0) {
                            *bst = *node_data;
                        } else if (node_data->total_time < bst->total_time) {
                            if (node_data->plan.cpu_frac != bst->plan.cpu_frac) {
                                *cur = *bst;
                            }
                            *bst = *node_data;
                        }
                        ...
                        // 单独统计纯CPU和纯iGPU耗时
                        if (node_data->plan.cpu_frac == 0) {
                            shared_data->shared->history->igpu = *node_data;
                        } else if (node_data->plan.igpu_frac == 0) {
                            shared_data->shared->history->cpu = *node_data;
                        }
                        ++shared_data->shared->history->opt_round;
                        pthread_spin_unlock(&shared_data->shared->history->spx);
                    }
                    __atomic_sub_fetch(shared_data->shared->work_node->n_wait, 1, __ATOMIC_SEQ_CST);
                }
                if (empty(shared_data->handler->jobs)) {
                    pthread_mutex_lock(lock);
                    shared_data->handler->n_wait = n_threads;
                    pthread_cond_broadcast(cv_main);
                    pthread_mutex_unlock(lock);
                }
                // 忙等待任务并分发
                while (empty(shared_data->handler->jobs)) {
                    usleep(0);
                    if (shared_data->handler->exit) {
                        return GGML_EXIT_SUCCESS;
                    }
                };

                shared_data->handler->n_wait = n_threads - 1;

                get_job(shared_data->handler->jobs, &shared_data->shared->work_node, &shared_data->shared->data, &shared_data->shared->history);
                struct ggml_tensor *node = shared_data->shared->work_node;

                // 记录起始时间
                if (node->time_us != NULL) {
                    *(node->time_us) = ggml_time_us();
                }

                int nth = ggml_get_n_tasks(node, n_threads);
                shared_data->shared->nth = nth;
                // 只需要一个线程做初始化
                if (GGML_OP_HAS_INIT[node->op]) {
                    params.type = GGML_TASK_INIT;
                    params.ith = 0;
        ...
        params.type = GGML_TASK_COMPUTE;
        params.ith = shared_data->ith;
        params.nth = shared_data->shared->nth;
        params.wsize = shared_data->handler->wsize;
        params.wdata = shared_data->handler->wbuff;
...
struct combo_thread_pool_handler *combo_create_thread_pool(int n_threads, uint32_t numa_cores) {
    struct combo_job_queue *job_que = create_combo_job_queue();
    struct combo_thread_pool_handler *handler =
        (struct combo_thread_pool_handler *)malloc(sizeof(struct combo_thread_pool_handler));
    *handler = (struct combo_thread_pool_handler){
        .n_threads = n_threads,
        .jobs = job_que,
        .exit = false,
        .n_wait = 0,
        .threads = malloc(sizeof(ggml_thread_t) * n_threads),
        .wsize = 0,
        .wbuff = NULL,
    };
    pthread_cond_init(&handler->cv, 0);
    pthread_mutex_init(&handler->lock, 0);

    struct combo_compute_state_shared *shared =
        (struct combo_compute_state_shared *)malloc(sizeof(struct combo_compute_state_shared));
    *shared = (struct combo_compute_state_shared){
        .work_node = NULL,
        .nth = 0,
    };

    struct combo_compute_state *workers = malloc(sizeof(struct combo_compute_state) * n_threads);
    for (int j = 0; j < n_threads; ++j) {
        workers[j] = (struct combo_compute_state){
            .ith = j,
            .numa_cores = numa_cores,
            .handler = handler,
            .shared = shared,
        };
        const int rc = ggml_thread_create(&workers[j].handler->threads[j], NULL,
                                          (void *(*)(void *))combo_compute_thread, (void *)&workers[j]);
        GGML_ASSERT(rc == 0);
        UNUSED(rc);
    }
    return handler;
...
int combo_del_thread_pool(struct combo_thread_pool_handler *handler) {
    pthread_mutex_lock(&handler->lock);
    if (handler->n_wait != handler->n_threads) {
        pthread_cond_wait(&handler->cv_main, &handler->lock);
    }
    handler->exit = true;
    pthread_cond_broadcast(&handler->cv);
    pthread_mutex_unlock(&handler->lock);
    for (int j = 0; j < handler->n_threads; j++) {
        const int rc = ggml_thread_join(handler->threads[j], NULL);
        GGML_ASSERT(rc == 0);
    }

    pthread_cond_destroy(&handler->cv);
    pthread_mutex_destroy(&handler->lock);
    free(handler->threads);
    delete_combo_job_queue(handler->jobs);
    free(handler);
    return 0;
}

int combo_pool_submit(struct combo_thread_pool_handler *handler, struct ggml_tensor *node,
                                   struct combo_tensor_data *data, struct combo_decode_history *history) {
    // wait for ready
    for (int i = GGML_MAX_SRC - 1; i >= 0; --i) {
        if (node->src[i] != NULL) {
            if (node->src[i]->n_wait != NULL) {
                while (__atomic_load_n(node->src[i]->n_wait, __ATOMIC_SEQ_CST) > 0) {
                    usleep(0);
                }
            }
        }
    }
    return add_job(handler->jobs, node, data, history);
}

GGML_API int combo_pool_set_cplan(struct combo_thread_pool_handler *handler, struct ggml_cplan *cplan) {
    handler->wsize = cplan->work_size;
    handler->wbuff = cplan->work_data;
    return 0;
}

int combo_pool_finish(struct combo_thread_pool_handler *handler) {
    while (empty(handler->jobs) == false) {
        pthread_mutex_lock(&handler->lock);
        if (handler->n_wait != handler->n_threads) {
            pthread_cond_wait(&handler->cv_main, &handler->lock);
        }
