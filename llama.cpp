...
static int32_t get_num_physical_cores() {
#ifdef __linux__
...
#else
        (void) n_gpu_layers;
#endif // defined(GGML_USE_CLBLAST)
    }
...
// combo
//////
static void combo_graph_compute(struct ggml_cgraph *cgraph, struct ggml_cplan *cplan);
static void combo_graph_compute_helper(std::vector<uint8_t> &buf, ggml_cgraph *graph, int n_threads) {
    struct ggml_cplan plan = ggml_graph_plan(graph, n_threads);
    ...
    combo_graph_compute(graph, &plan);
}
...
// return negative int on error
//
static int combo_decode_internal(llama_context &lctx, llama_batch batch) {
    const uint32_t n_tokens = batch.n_tokens;
    ...
    // after enough generations, the benefit from this heuristic disappears
    // if we start defragmenting the cache, the benefit from this will be more important
    // kv_self.n = std::max(32, GGML_PAD(llama_kv_cache_cell_max(kv_self), 32));   // TODO: this might be better for
    // CUDA?
    kv_self.n = std::min((int32_t)cparams.n_ctx, std::max(32, llama_kv_cache_cell_max(kv_self)));
    ...
    combo_graph_compute_helper(lctx.work_buffer, gf, n_threads);
    ...
    //       need to update the graphs to skip "result_output"
    {
        ...
        if (batch.logits) {
            ...
            for (uint32_t i = 0; i < n_tokens; i++) {
                ...
                memcpy(logits_out.data() + (n_vocab * i), (float *)ggml_get_data(res) + (n_vocab * i),
            ...
            logits_out.resize(n_vocab);
            memcpy(logits_out.data(), (float *)ggml_get_data(res) + (n_vocab * (n_tokens - 1)),
                   sizeof(float) * n_vocab);
        }
    }
    ...
    if (!lctx.embedding.empty()) {
        ...
        embedding_out.resize(n_embd);
        memcpy(embedding_out.data(), (float *)ggml_get_data(embeddings) + (n_embd * (n_tokens - 1)),
               sizeof(float) * n_embd);
    }
...
int combo_decode(struct llama_context *ctx, struct llama_batch batch) {
    const int ret = combo_decode_internal(*ctx, batch);
    if (ret < 0) {
        LLAMA_LOG_ERROR("%s: failed to decode, ret = %d\n", __func__, ret);
    }
...
// append decode info to file: src1 @ src0 -> dst | cpu_frac, igpu_frac | cpu_time, igpu_time
static void print_decode_info(struct ggml_cgraph *graph, std::vector<struct combo_tensor_data> &data,
                              const char *file_name = "decode_info.csv") {
    FILE *fp = fopen(file_name, "a+");
    fprintf(fp, "node index,node type-src1 type-src0 type-src1_shape@src0_shape,cpu frac,igpu frac,cpu us,igpu us\n");
    int n_node_mul_mat = 0;
    for (int i = 0; i < graph->n_nodes; i++) {
        struct ggml_tensor *node = graph->nodes[i];
        if (combo_is_mul_mat(node)) {
            fprintf(fp, "%d,%d %d %d (%ld %ld %ld %ld)@(%ld %ld %ld %ld),%d,%d,%ld,%ld\n", i, node->type,
                    node->src[1]->type, node->src[0]->type, node->src[1]->ne[3], node->src[1]->ne[2],
                    node->src[1]->ne[1], node->src[1]->ne[0], node->src[0]->ne[3], node->src[0]->ne[2],
                    node->src[0]->ne[0], node->src[0]->ne[1], data[n_node_mul_mat].plan.cpu_frac,
                    data[n_node_mul_mat].plan.igpu_frac, data[n_node_mul_mat].cpu_time, data[n_node_mul_mat].igpu_time);
            n_node_mul_mat++;
        }
    }
    fclose(fp);
...
static std::map<std::string, struct combo_decode_history> decode_stat_map;

struct combo_compute_plan_source {
    struct ggml_tensor *node;
    struct combo_decode_history *tensor_history;
};

char combo_encode_tmp[256];
inline static std::string combo_encode_tensor(struct ggml_tensor *node) {
    int rt = std::sprintf(combo_encode_tmp, "%d|%d|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld", node->op, node->type,
                          node->src[1]->ne[3], node->src[1]->ne[2], node->src[1]->ne[1], node->src[1]->ne[0],
                          node->src[0]->ne[3], node->src[1]->ne[2], node->src[1]->ne[0], node->src[1]->ne[1]);
    GGML_ASSERT(rt >= 0);
    return std::string(combo_encode_tmp);
}

// divide tensor to column parallelization
int combo_tensor_col_divide(struct combo_compute_plan *plan, struct ggml_tensor *data, struct ggml_tensor *cpu_tensor,
                            struct ggml_tensor *igpu_tensor) {
    struct ggml_tensor *cpu_src0 = cpu_tensor->src[0], *igpu_src0 = igpu_tensor->src[0];
    int n_tasks = plan->cpu_frac + plan->igpu_frac;
    // element of src0 dimension 1
    int ne01 = data->src[0]->ne[1];
    int nrow_igpu = ne01 * plan->igpu_frac / n_tasks;
    nrow_igpu = nrow_igpu < 4 ? 0 : nrow_igpu;
    int nrow_cpu = ne01 - nrow_igpu;
    GGML_ASSERT(nrow_igpu >= 0);
    GGML_ASSERT(nrow_cpu >= 0);
    GGML_ASSERT(nrow_igpu + nrow_cpu == ne01);

    if (nrow_igpu != 0) {
        igpu_tensor->ne[0] = igpu_src0->ne[1] = nrow_igpu;
    } else {
        igpu_tensor->data = NULL;
    }

    if (nrow_cpu != 0) {
        cpu_tensor->ne[0] = cpu_src0->ne[1] = nrow_cpu;
        cpu_src0->data = (char *)(cpu_src0->data) + nrow_igpu * igpu_src0->nb[1];
        int off = nrow_igpu * igpu_tensor->nb[0] / ggml_blck_size(igpu_tensor->type);
        cpu_tensor->data = (char *)cpu_tensor->data + off;
    } else {
        cpu_tensor->data = NULL;
    }

    return 0;
}

inline static bool cl_can_mul_mat(const struct ggml_tensor *src0, const struct ggml_tensor *src1,
                                  struct ggml_tensor *dst) {
#ifdef GGML_USE_CLBLAST
    return combo_cl_can_mul_mat(src0, src1, dst);
#else
    return false;
#endif
}

struct combo_thread_pool_handler *cpu_thread_pool = nullptr;
struct combo_thread_pool_handler *igpu_thread_pool = nullptr;
static combo_plan *plan_algo = nullptr;

int combo_init(const int n_threads, const int algo) {
    if (algo == kAlgoGD) {
        plan_algo = new combo_plan_gd;
    } else if (algo == kAlgoColdStart) {
        plan_algo = new combo_plan_balance;
    } else if (algo == kAlgoConst) {
        plan_algo = new combo_plan_const(kAlgoConst);
    } else if (algo == kAlgoCPUOnly) {
        plan_algo = new combo_plan_const(kAlgoCPUOnly);
    } else if (algo == kAlgoiGPUOnly) {
        plan_algo = new combo_plan_const(kAlgoiGPUOnly);
    } else {
        GGML_ASSERT(false);
    }
#ifdef COMBO_DEV
    fprintf(stderr, "COMBO DEV MODE: ON\n");
#else
    fprintf(stderr, "COMBO DEV MODE: OFF\n");
#endif
    uint32_t numa_cores = get_num_physical_cores();
    igpu_thread_pool = combo_create_thread_pool(1, numa_cores);
    cpu_thread_pool = combo_create_thread_pool(n_threads, numa_cores);
    return 0;
}
...
int combo_finish() {
    ...
    combo_del_thread_pool(cpu_thread_pool);
    combo_del_thread_pool(igpu_thread_pool);
    return 0;
}
...
static void combo_graph_compute(struct ggml_cgraph *cgraph, struct ggml_cplan *cplan) {
    ...
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        if (combo_is_mul_mat(cgraph->nodes[i])) {
            n_mul_mat += 1;
        }
    }
    std::vector<struct combo_tensor_data> compute_data(n_mul_mat, {{0, 0}, 0, 0, 0});

    int n_divieds = 0;
    std::vector<struct ggml_tensor> tensors(4 * n_mul_mat);

    struct ggml_tensor *cpu_tensor, *cpu_src0;
    struct ggml_tensor *igpu_tensor, *igpu_src0;

    n_mul_mat = 0;
    combo_pool_set_cplan(cpu_thread_pool, cplan);

    std::vector<int> n_waits(cgraph->n_nodes, 1);

    for (int node_n = 0; node_n < cgraph->n_nodes; ++node_n) {
        struct ggml_tensor *node = cgraph->nodes[node_n];
        node->n_wait = &n_waits[node_n];
        node->time_us = NULL;
        if (!combo_is_mul_mat(node)) {
            combo_pool_submit(cpu_thread_pool, node, NULL, NULL);
        } else {
            struct combo_tensor_data &cur_data = compute_data[n_mul_mat++];
            struct combo_decode_history *tensor_history = nullptr;
            tensor_history = &plan_algo->get_tensor_history(node);
            plan_algo->make_plan(node, *tensor_history, &cur_data.plan);
            if (cur_data.plan.igpu_frac == 0) {
                node->op = COMBO_OP_MUL_MAT_CPU;
                node->time_us = &(cur_data.cpu_time);
                combo_pool_submit(cpu_thread_pool, node, &cur_data, tensor_history);
            } else if (cur_data.plan.cpu_frac == 0) {
                node->op = COMBO_OP_MUL_MAT_OPENCL;
                node->time_us = &cur_data.igpu_time;
                combo_pool_submit(igpu_thread_pool, node, &cur_data, tensor_history);
            } else {
                // combo compute
                cpu_src0 = &tensors[n_divieds++];
                memcpy(cpu_src0, node->src[0], sizeof(struct ggml_tensor));
                cpu_tensor = &tensors[n_divieds++];
                memcpy(cpu_tensor, node, sizeof(struct ggml_tensor));
                cpu_tensor->src[0] = cpu_src0;

                igpu_src0 = &tensors[n_divieds++];
                memcpy(igpu_src0, node->src[0], sizeof(struct ggml_tensor));
                igpu_tensor = &tensors[n_divieds++];
                memcpy(igpu_tensor, node, sizeof(struct ggml_tensor));
                igpu_tensor->src[0] = igpu_src0;

                combo_tensor_col_divide(&cur_data.plan, node, cpu_tensor, igpu_tensor);
                if (cl_can_mul_mat(igpu_tensor->src[0], igpu_tensor->src[1], igpu_tensor)) {
                    if (cpu_tensor->data != NULL) {
                        *(node->n_wait) = 2;
                        cpu_tensor->op = COMBO_OP_MUL_MAT_CPU;
                        cpu_tensor->time_us = &cur_data.cpu_time;
                        combo_pool_submit(cpu_thread_pool, cpu_tensor, &cur_data, tensor_history);
                    }
                    igpu_tensor->op = COMBO_OP_MUL_MAT_OPENCL;
                    igpu_tensor->time_us = &cur_data.igpu_time;
                    combo_pool_submit(igpu_thread_pool, igpu_tensor, &cur_data, tensor_history);
                } else {
                    cpu_tensor->op = COMBO_OP_MUL_MAT_CPU;
                    cpu_tensor->time_us = &cur_data.cpu_time;
                    cur_data.plan.cpu_frac = 100;
                    cur_data.plan.igpu_frac = 0;
                    combo_pool_submit(cpu_thread_pool, cpu_tensor, &cur_data, tensor_history);
                }
            }
        }
    }
    combo_pool_finish(cpu_thread_pool);
    combo_pool_finish(igpu_thread_pool);
#ifdef COMBO_DEV
    print_decode_info(cgraph, compute_data);
#endif
}

int combo_plan_balance::make_plan(struct ggml_tensor *node, struct combo_decode_history &history,
                                  struct combo_compute_plan *plan) {
    if (!combo_is_mul_mat(node)) {
        plan->cpu_frac = 100;
        plan->igpu_frac = 0;
        return 0;
    ...
    pthread_spin_lock(&history.spx);
    struct combo_tensor_data cpu = history.cpu, igpu = history.igpu;
    pthread_spin_unlock(&history.spx);
    ...
    if (cpu.total_time == 0) {
        ...
        plan->igpu_frac = 0;
        return 0;
    } else if (igpu.total_time == 0) {
        plan->cpu_frac = 0;
        plan->igpu_frac = 100;
        return 0;
    }
    // balance workload of cpu and igpu
    float cpu_frac = (float)igpu.total_time / (cpu.total_time + igpu.total_time);
    cpu_frac = cpu_frac >= 0.9 ? 1 : cpu_frac;
    cpu_frac = cpu_frac <= 0.1 ? 0 : cpu_frac;
    plan->cpu_frac = cpu_frac * 100;
    plan->igpu_frac = 100 - plan->cpu_frac;
    return 0;
}

int combo_plan_gd::make_plan(struct ggml_tensor *node, struct combo_decode_history &history,
                             struct combo_compute_plan *plan) {
    if (!combo_is_mul_mat(node)) {
        plan->cpu_frac = 100;
        plan->igpu_frac = 0;
    ...
    pthread_spin_lock(&history.spx);
    struct combo_tensor_data cpu = history.cpu, igpu = history.igpu, bst = history.bst, current = history.current;
    uint64_t opt_round = history.opt_round;
    pthread_spin_unlock(&history.spx);
    ...
    // 强制初始化数据
    if (igpu.total_time == 0) {
        plan->cpu_frac = 0;
        plan->igpu_frac = 100;
        return 0;
    } else if (cpu.total_time == 0) {
        plan->cpu_frac = 100;
        plan->igpu_frac = 0;
        return 0;
    }
    ...
    float cpu_frac = 1;
    auto set_plan = [&cpu_frac, &plan]() -> int {
        cpu_frac = cpu_frac >= 1 - kComboStepMin ? 1 : cpu_frac;
        cpu_frac = cpu_frac <= kComboStepMin ? 0 : cpu_frac;
        plan->cpu_frac = cpu_frac * 100;
        plan->igpu_frac = 100 - plan->cpu_frac;
        return 0;
    };
    ...
    // cold start
    if (opt_round == 2) {
        cpu_frac = (float)igpu.total_time / (cpu.total_time + igpu.total_time);
        set_plan();
        if (plan->cpu_frac == 0 || plan->cpu_frac == 100) {
            if (cpu.total_time < igpu.total_time) {
                cpu_frac = 1.0 - kComboStepMin * 2;
            } else {
                cpu_frac = kComboStepMin * 2;
            }
            return set_plan();
        }
        return 0;
    ...
    const float bst_cpu_frac = (float)bst.plan.cpu_frac / (bst.plan.cpu_frac + bst.plan.igpu_frac);
    // const float current_cpu_frac = ((float)current.plan.cpu_frac) / (current.plan.cpu_frac + current.plan.igpu_frac);
    const float frac_diff = (float)current.plan.cpu_frac - bst.plan.cpu_frac;
    const float time_diff = (float)current.total_time - bst.total_time;
    const float grad = time_diff / frac_diff;

    if (opt_round == 3) {
        cpu_frac = grad > 0 ? bst_cpu_frac - kComboStepMin : bst_cpu_frac + kComboStepMin;
        return set_plan();
    }

    if (time_diff <= kComboDampingAbs || std::abs(grad) <= kComboDampingRate) {
        *plan = bst.plan;
        return 0;
    }

    float step = kComboStepMax / ((float)opt_round / 5 + 1);
    step = kComboStepMin > step ? kComboStepMin : step;
    cpu_frac = grad > 0 ? bst_cpu_frac - step : bst_cpu_frac + step;
    ...
    // if (plan->cpu_frac == current.plan.cpu_frac) {
    //     cpu_frac = grad > 0 ? bst_cpu_frac - kComboStepMin : bst_cpu_frac + kComboStepMin;
    //     return set_plan();
    // }
    return 0;
...
int combo_plan_const::make_plan(struct ggml_tensor *node, struct combo_decode_history &history,
                                struct combo_compute_plan *plan) {
    if (!combo_is_mul_mat(node)) {
        plan->cpu_frac = 100;
        plan->igpu_frac = 0;
    ...
    if (mode_now_ == kAlgoConst) {
        plan->cpu_frac = 50;
        plan->igpu_frac = 50;
        return 0;
    } else if (mode_now_ == kAlgoCPUOnly) {
        plan->cpu_frac = 100;
        plan->igpu_frac = 0;
        return 0;
    } else if (mode_now_ == kAlgoiGPUOnly) {
        plan->cpu_frac = 0;
        plan->igpu_frac = 100;
        return 0;
    }
